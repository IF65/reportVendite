...
if ($argc == 1) {
	...
	$dataFine = new DateTime('2021-02-15', $timeZone);
} elseif ($argc == 2) {
	$sede = $argv[1];
	$dataInizio = new DateTime('2021-02-15', $timeZone);
	$dataFine = new DateTime('2021-02-15', $timeZone);
	...
	$sede = $argv[1];
	$dataInizio = new DateTime($argv[2], $timeZone);
	$dataFine = new DateTime($argv[2], $timeZone);
} else {
	$sede = $argv[1];
	$dataInizio = new DateTime($argv[2], $timeZone);
	$dataFine = new DateTime($argv[3], $timeZone);
}
...
try {
    ...
    while ($data <= $dataFine) {
	    ...
	    $h_count_sales_query->execute([':ddate' => $data->format('Y-m-d'), ':store' => $sede]);
	    $value = $h_count_sales_query->fetch(PDO::FETCH_ASSOC);
	    $salesRecordCount = $value['recordCount'] * 1;

	    if (!$salesRecordCount) {
		    $h_load_query->execute(['data' => $data->format('Y-m-d'), ':store' => $sede]);
